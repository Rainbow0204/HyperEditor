...
from criteria import id_loss, moco_loss, clip_loss, image_embedding_loss
from configs import data_configs
from datasets.images_dataset import ImagesDataset
from datasets.latents_images_dataset import LatentsImagesDataset
from criteria.lpips.lpips import LPIPS
from models.hyperEditor import HyperEditor
from training.ranger import Ranger


def loss_clip(img, text):
    clip_model, preprocess = clip.load("ViT-B/32", device="cuda")
    device = "cuda" if torch.cuda.is_available() else "cpu"
    with torch.no_grad():
        text = clip.tokenize(text).to(device)
        text = text.repeat(img.shape[0], 1)
        image_features = clip_model.encode_image(img)
        text_features = clip_model.encode_text(text)
    loss = 1 - torch.cosine_similarity(image_features, text_features, dim=1)
    return loss
...
class Coach:
    def __init__(self, opts):
        ...
        # Initialize network
        self.net = HyperEditor(self.opts).to(self.device)
    ...
    def parse_batch(self, batch):
        x, y, style_fix, y_hat, latents = None, None, None, None, None
        if isinstance(self.train_dataset, ImagesDataset):
            x, y, style_fix = batch   #x为原图，y为resize后的结果
            x, y, style_fix = x.to(self.device).float(), y.to(self.device).float(), style_fix.to(self.device).float()
        ...
        return x, y, style_fix, y_hat, latents

    def perform_forward_on_batch(self, batch, condition=None, train=False):
        latent, weights_deltas, w_inversion, initial_inversion = None, None, None, None
        cur_loss_dict, id_logs = None, None
        x, y, style_fix, y_hat, codes = self.parse_batch(batch)
        ...
        for iter in range(self.opts.n_iters_per_batch):
            ...
            y_hat, latent, weights_deltas, codes, w_inversion, delta_i, globe_clip = self.net.forward(x,
                                                                                 condition=condition,
                                                                                 y_hat=y_hat,
                                                                                 codes=codes,
            ...
            for idx in range(x.shape[0]):
                if id_logs is None:
                    ...
                    y_hats[idx].append([y_hat[idx].detach().cpu(), 0])
                else:
                    y_hats[idx].append([y_hat[idx].detach().cpu(), id_logs[idx]['diff_target']])
        return x, y, style_fix, y_hats, cur_loss_dict, id_logs, initial_inversion
    ...
    def train(self):
        ...
        index = 0
        condition = []
        if self.opts.target_text_file is not None:
            with open(self.opts.target_text_file, "r") as templates:
                condition = templates.readlines()
            index_max = len(condition)
        while self.global_step < self.opts.max_steps:
            for batch_idx, batch in enumerate(self.train_dataloader):
                if self.opts.target_text_file is not None:
                    if index >= index_max:
                        index = 0
                    condition_text = [condition[index], self.opts.init_text]
                    index += 1
                else:
                    condition_text = [self.opts.target_text, self.opts.init_text]

                self.optimizer.zero_grad()
                x, y, style_fix, y_hat, loss_dict, id_logs, w_inversion = self.perform_forward_on_batch(batch, condition=condition_text, train=True)
                self.optimizer.step()

                # Logging related
                if self.global_step % self.opts.image_interval == 0 or (self.global_step < 10000 and self.global_step % 25 == 0):#
                    self.parse_and_log_images(id_logs, x, y, style_fix, y_hat, w_inversion, title='images/train')
    ...
    def calc_loss(self, x, y, style_fix, y_hat, latent, weights_deltas, w_inversion, delta_i=None, delta_s=None):
        loss_dict = {}
        loss = 0.0
        id_logs = None
        ...
        if "cars" in self.opts.dataset_type:
            ...
            style_fix_resized = torch.nn.AdaptiveAvgPool2d((192, 256))(style_fix)
            w_inversion_resized = torch.nn.AdaptiveAvgPool2d((192, 256))(w_inversion)
        else:
            y_hat_resized = self.net.face_pool(y_hat)
            y_resized = self.net.face_pool(y)
            x_resized = self.net.face_pool(x)
            style_fix_resized = self.net.face_pool(style_fix)
            w_inversion_resized = self.net.face_pool(w_inversion)
        ...
        if self.opts.lpips_lambda > 0:
            loss_lpips = self.lpips_loss(y_hat_resized, w_inversion_resized)
        ...
        if self.opts.clip_sim > 0:
            loss_clip = self.loss_clip(
                masked_generated=y_hat_resized,
                masked_img_tensor=w_inversion_resized,
                delta_i=delta_i, delta=True).mean()
            loss_dict['loss_clip'] = float(loss_clip) * self.opts.clip_sim
            loss += loss_clip * self.opts.clip_sim
    ...
    def parse_and_log_images(self, id_logs, x, y, style_fix, y_hat, w_inversion, title, subscript=None, display_count=2):
        im_data = []
        ...
        for i in range(display_count):
            ...
            cur_im_data = {
                ...
                'style_fix': common.tensor2im(style_fix[i]),
